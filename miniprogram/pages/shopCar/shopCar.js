...
Page({
    ...
    createOrder() {
        ...
        let payment = {
            ...
            totalFee: that.accMul(this.data.totalPrice, 100) + (this.data.totalPrice < 78 ? 500 : 0), //订单总价，订单总价=商品总金额+餐盒费+配送费，单位：分
            deliveryFee: (this.data.totalPrice < 78 ? 500 : 0), //配送费
            packageFee: 0, //餐盒费
            discountFee: 0,
            platformDiscountFee: 0,
            shopDiscountFee: 0,
            shopFee: that.accMul(this.data.totalPrice, 100) + (this.data.totalPrice < 78 ? 500 : 0), //商户实收总价
            userFee: that.accMul(this.data.totalPrice, 100) + (this.data.totalPrice < 78 ? 500 : 0), //用户实付总价
            serviceFee: 0,
            subsidies: 0,
    ...
    handlePayment() {
        ...
        let data = {
            ...
            totalFee: that.data.totalPrice * 100 + (this.data.totalPrice < 78 ? 500 : 0)
        }
